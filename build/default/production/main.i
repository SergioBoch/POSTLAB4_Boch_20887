...
REINICIO MACRO ; Reinicio del timer 0. Funciona paralelo al programa
    ...
    MOVWF TMR0
    BCF ((INTCON) and 07Fh), 2 ; Limpiando la bandera del timer 0.
    ENDM
...
isr: ; Rutina principal de reseteo
    BTFSC ((INTCON) and 07Fh), 2 ; Verifica la bandera del timer 0
    call General_Int
...
General_Int: ; Rutina de interrupción general
    ...
    BTFSC ((STATUS) and 07Fh), 2 ; Verificar que esta en 0
...
Display2:
    ...
    BTFSC ((STATUS) and 07Fh), 2 ; Al llegar a 60s se reincia
    call MAXIMO
    RETURN
